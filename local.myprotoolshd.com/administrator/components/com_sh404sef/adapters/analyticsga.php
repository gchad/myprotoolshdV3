...
class Sh404sefAdapterAnalyticsga extends Sh404sefClassBaseanalytics
{
	...
	protected function _fetchAccountsList()
	{
		...
		{
			throw new Sh404sefExceptionDefault('Analytics: no classic analytics web property ID set!');
		}
		...
		if ($config->analyticsEdition == 'gtm' && empty($config->analyticsId) && empty($config->analyticsUgaId))
		{
			throw new Sh404sefExceptionDefault('Analytics: using Google tags manager, but no classic or universal web property ID set!');
		}
		if (($config->analyticsEdition == 'ga' || $config->analyticsEdition == 'ga_and_uga'))
		{
			$id = $config->analyticsId;
		}
		else if ($config->analyticsEdition == 'uga')
		{
			$id = $config->analyticsUgaId;
		}
		else if ($config->analyticsEdition == 'gtm')
		{
			$id = $config->analyticsId;
			$id = empty($id) ? $config->analyticsUgaId : $id;
		}
		...
		$accountIdBits = explode('-', trim($id));
		//echo '<br />Analytics id used: ' . $id . '(' . count($accountIdBits) . ')<br />' . print_r($accountIdBits, true);
		...
		// set target API url
		$uri = $this->_endPoint . 'management/accounts/' . $accoundId . '/webproperties/' . trim($id) . '/profiles';
