...
class itemlistController extends JControllerLegacy
{
	...
	function published_story(){
		...
		$db = JFactory::getDbo();
		$query1 = 'UPDATE #__je_k2itemlist'. ' SET published = ' .$id. ' WHERE id ='.$uid; 
		$db->setQuery( $query1 );
		$db->query();
		...
		$mainframe->redirect ( 'index.php?option='.$option.'&view=itemlist' );		
	}
	function delete()
	{
		...
		$query = 'DELETE FROM #__je_k2itemlist WHERE itemid IN ( '.$cids.' )';
		$db->setQuery( $query );
		$db->query();
	...
	{ 
		...
		$sel_data = array();
		$sel_data[]  = JHTML::_('select.option', '0 ', JText::_( 'SELECT_ITEM'));
