...
$loadSidebarCSS = '';
if(!$sidebar_load_open)//close the sider with proper margin left/right
{
    if($sidebar_position == 'left')
        $loadSidebarCSS .= 'margin-left:-'.($sidebar_width+11).'px;';
    else if($sidebar_position == 'right')
        $loadSidebarCSS .= 'margin-right:-'.($sidebar_width+11).'px;';
}
if(!$sidebar_enable)
{
...
?>
<?php
if($results_enable && $results_position == 'top')
{
?>
    <div class="markerStatisticsParent">
        <div class="markerStatistics clearfix" id="markerStatistics" style="<?php echo ($results_type == "byzoom")?"display: none;":""; ?>">
        <?php 
...
<div id="map-animate-box">
    <?php
    $flashMessageStyle = "";
    if(!$sidebar_enable || $sidebar_position == "" || $sidebar_position == 'left')
    {
    ...
    <div style="position: relative;overflow: hidden;">
        ...
        if($contents_enable && $show_global_notice && ($sidebar_position == $notice_position))
        {
            $sidebarHeight = $sidebarHeight - $notice_offset;
        }
        else if($contents_enable)
        {
            $sidebarHeight = $sidebarHeight - 40;
        }
        if(!empty($allowedPluginsInModule))
        {
            $sidebarHeightStyle = !$allTabsAreClosed?'height:'.$sidebarHeight.'px;':'';
        ?>
            <div id="map-canvas-sidebar" class="sidebarAligned<?php echo $sidebar_position; ?>" style="position:absolute;z-index:100;width:<?php echo $sidebar_width; ?>px;<?php echo $sidebarHeightStyle; ?><?php echo $loadSidebarCSS; ?>" data-original_height="<?php echo $sideBarHeight.'px'; ?>">
                <div class="map-canvas-sidebar-inner"<?php echo $allTabsAreClosed?" style='background:none;'":"";?>>            
        			<div class="toolsbar-area clearfix">
        				<ul>
                            ...
                            if($buttonsenabled_userposition)
                            {
                            ?>
                            ...
                            if($mobilebuttons_listview && !$allTabsAreClosed)
                            {
                            ?>
                                <li>
                    ...
                    <div class="toolsbar-area-vertical" style="position:absolute;top:10px;<?php echo $sidebar_position; ?>:<?php echo $sidebar_width+10; ?>px;<?php echo ($sidebar_load_open)?'display:none;':''; ?>" id="closed_sidebar_toolbar">
        				<ul>
                        ...
                        <div class="titles-tab-area">
                            ...
                            if(!empty($filters))
                            {
                                ...
                                foreach($filters as $filter)
                                {
                                    ...
                                    if($tabLIs == '')
                                    {
                                        $activeTabClass = !$contents_enable?' active':'';
                                ...
                                <div class="tab-content" id="plugin_tabs_contents"<?php echo ($allTabsAreClosed)?" style='display:none;'":""; ?>>
                                  ...
                                  ?>
                                    <div class="tab-pane active" id="title_tab_all">
                                        <?php
                                        if($show_global_notice && ($sidebar_position == $notice_position))
                                        {
                                            $global_result_height = $global_result_height - $notice_offset - 20;
                                        }
                                        else
                                        {
                                            $global_result_height = $global_result_height - 70;
                                        }
                                        ?>
                                        <div id="global_result_list" class="" style="height: <?php echo $global_result_height; ?>px;"></div>                                                                  
                                    </div>
                                  <?php
                                  }
                                  ?>  
        ...
        if($show_global_notice)
        {
        ?>
            <div id="notice_box_holder_global" class="notice_box_holder noticePositions<?php echo $notice_position.$infoLinkClass; ?>" style="width:<?php echo $sidebar_width; ?>px;">
        ...
        else if($pluginNoticeExist)
        {
            $dispatcher->trigger('onNoticeAreaDisplay');
        }
        ?>
        <div id="map-canvas" style="width:<?php echo $map_dimensions_width; ?>;height:<?php echo $map_dimensions_height; ?>px;">
    ...
    if($infolink_enable)
    {
        $infolink_url = ($infolink_url != "")?$infolink_url:"javascript:void(0);";
    ?>
        <div class="infoLinkWrapper infolinkAligned<?php echo $sidebar_position; ?>">
    ...
    ?>    
    <?php
    if($results_enable && $results_position == 'bottom')
    {
    ?>
        <div class="markerStatisticsParent">
            <div class="markerStatistics clearfix" id="markerStatistics" style="<?php echo ($results_type == "byzoom")?"display: none;":""; ?>">
