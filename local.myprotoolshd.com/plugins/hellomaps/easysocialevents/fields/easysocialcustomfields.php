...
$filecommunity 	= JPATH_ADMINISTRATOR . '/components/com_easysocial/includes/foundry.php';
	...
	if( !JFile::exists( $filecommunity ) )
		{
			echo "<div class='alert alert-error' style='font-size: 18px;'>".JText::_('PLG_HELLOMAPS_EASYSOCIALEVENTS_CHECK_FILE')."</div>";
			return;
...
$filecommunity 	= JPATH_ADMINISTRATOR . '/components/com_easysocial/includes/foundry.php';
	...
	if( !JFile::exists( $filecommunity ) )
		{
			echo "<div class='alert alert-error' style='font-size: 18px;'>".JText::_('PLG_HELLOMAP_EVENTS_CHECK_FILE')."</div>";
			return;
 ...
 */
if(!class_exists('Easysocialcustomfields'))
{	
	class JFormFieldEasysocialcustomfields extends JFormFieldList
	{
		/**
		 * The field type.
		 *
		 * @var		string
		 */
		protected $type = 'Easysocialcustomfields';
		...
		protected function getOptions() 
		{
			...
			if ($items)
			{			
				//$options[] = JHtml::_('select.option', 'username', JText::_('PLG_HELLOMAPS_EASYSOCIALEVENTS_USERNAME'));//put the username field
                //$options[] = JHtml::_('select.option', 'useremail', JText::_('PLG_HELLOMAPS_EASYSOCIALEVENTS_EMAIL'));//put the username field
				foreach($items as $item) 
				{
					
					if (($item->text!= "COM_EASYSOCIAL_FIELDS_EVENT_ADDRESS")&&($item->text!= "COM_EASYSOCIAL_FIELDS_EVENT_RECURRING"))
					{
