 ...
 */
if(!class_exists('JFormFieldEasysocialEventsmarkers'))
{	
	class JFormFieldEasysocialEventsmarkers extends JFormField
	{
		/**
		 * The field type.
		 *
		 * @var		string
		 */
		protected $type = 'easysocialeventsmarkers';
		...
		{		
			...
			$document->addScript(JURI::root().'plugins/hellomaps/easysocialevents/fields/easysocialeventsmarkers.js');
					
			$query	= 'SELECT title AS text, id AS value, type '
						." FROM #__social_clusters_categories scc "
						//.' LEFT JOIN '.($db->quoteName('#__social_clusters', 'a').'ON a.id = scc.field_id')
						." WHERE type = 'event' "
			...
			$fileOptions = array();
			$markersFolder = JPATH_ROOT.'/plugins/hellomaps/easysocialevents/images/markers';
			...
			$profileTypesHTML = '<ul class="easysocialeventsmarkers" style="list-style:none;">';
