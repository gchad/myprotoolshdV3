	...
	jQuery(document).ready(function() {
		...
		jQuery("input.fileUploadMultiple").on("change", function() {
			...
			for (var i = 0; i < jQuery(this).get(0).files.length; ++i) {
				selected.push(jQuery(this).get(0).files[i].name);
			}
			...
			var isFirstBreak = 0;
			jQuery(selected).each(function(k, file) {
				if(k == 0 && stopFirst == 1) {
					isFirstBreak = 1;
				}
				else {
				...
				if(isFirstBreak == 1) {
					jQuery('div.itemImages table.admintable:last-child input.fileUpload').val("asd");
				}
				else {
					jQuery("div.itemImages").append("<p>&nbsp;</p><p><?php echo JText::_('PLG_K2_MULTIIMAGES_IMAGE'); ?> "+newImage+"</p><hr />");
					jQuery('div.itemImages table.admintable:eq(0)').clone().appendTo("div.itemImages");
					...
					//jQuery('div.itemImages table.admintable:last-child input.fileUpload').val("qwe");
				}
				newImage++;
			});
								...
								<div class="simpleTabsContent" id="k2Tab2">
									...
									<?php else: ?>
										...
										<div class="multipleUpload" style="font-size: 12px;">
											...
											<input multiple type="file" name="image_multiple" class="fileUploadMultiple" accept="image/gif, image/jpeg, image/png" />
										</div>
									<?php endif; ?>
