...
class K2ViewItem extends K2View
{
	...
	//added
    {
		...
		if($item->image_caption != '') {
			$captions = explode("|", $item->image_caption);
		}
		if($item->image_credits != '') {
			$credits = explode("|", $item->image_credits);
		}
		...
		while(
			...
			if (JFile::exists($filepath.'_L.jpg')){
				...
				if($filecount > 1) {
					$item->$name = JURI::root().'media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_L.jpg'.$timestamp;
				}
				else {
					$item->$name = JURI::root().'media/k2/items/cache/'.md5("Image".$item->id).'_L.jpg'.$timestamp;
				}
			}

			if (JFile::exists($filepath.'_S.jpg')){
				$name = "thumb".$filecount;
				if($filecount > 1) {
					$item->$name = JURI::root().'media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_S.jpg'.$timestamp;
				}
				else {
					$item->$name = JURI::root().'media/k2/items/cache/'.md5("Image".$item->id).'_S.jpg'.$timestamp;
				}
			}
			...
			$item->$capt_n = '';
			if(!empty($captions)) {
				$item->$capt_n = $captions[$count];
			}

			$cred_n = "image".$filecount."_credits";
			$item->$cred_n = '';
			if(!empty($credits)) {
				$item->$cred_n = $credits[$count];
			}
		...
		$this->setLayout("itemform_multiimages");
        parent::display($tpl);
    }
