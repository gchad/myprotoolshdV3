...
//added
{
	...
	function prepareItem($item, $view, $task)
	{
		...
		/*
		$query = "SHOW TABLES LIKE '%k2_multiimages%'";
		$db->setQuery($query);
		$db_result = $db->loadResult();
		
		if($db_result) {
			$query = "SELECT * FROM #__k2_multiimages WHERE itemid = {$item->id}";
			$db->setQuery($query);
		...
		*/
		...
		(JFile::exists(JPATH_SITE.DS.'media'.DS.'k2'.DS.'items'.DS.'src'.DS.md5("Image".$item->id).".jpg") && $filecount == 1)
			...
			//Image multi
			$xsmall = "image".$filecount."XSmall";
			$item->$xsmall = '';
			
			$small = "image".$filecount."Small";
			$item->$small = '';
			
			$medium = "image".$filecount."Medium";
			$item->$medium = '';
			
			$large = "image".$filecount."Large";
			$item->$large = '';
			
			$xlarge = "image".$filecount."XLarge";
			$item->$xlarge = '';			
			
			$generic = "image".$filecount."Generic";
			$item->$generic = '';
			...
			if($filecount > 1) {
				$filepath = JPATH_SITE.DS.'media'.DS.'k2'.DS.'items'.DS.'cache'.DS.md5("Image".$item->id)."_".$filecount;
			}
			else {
				$filepath = JPATH_SITE.DS.'media'.DS.'k2'.DS.'items'.DS.'cache'.DS.md5("Image".$item->id);
			}

			if (JFile::exists($filepath.'_XS.jpg'))
			{
				if($filecount == 1) {
					$item->$xsmall = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_XS.jpg';
					$item->imageXSmall = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_XS.jpg';
				}
				else {
					$imageXsWidth = $params->get('itemImageXS', '100');
					//if($imageXsWidth <= '100') {
					//	$image = JPATH_SITE.DS.'/media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_XS.jpg';
					//	$imageData = base64_encode(file_get_contents($image));
					//	$item->$xsmall = 'data: image/jpeg;base64,'.$imageData;
					//}
					//else {
						$item->$xsmall = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_XS.jpg';
					//}
				}
								
				if ($params->get('imageTimestamp'))
				{
					$item->$xsmall .= $timestamp;
				}
			}

			if (JFile::exists($filepath.'_S.jpg'))
			{
				if($filecount == 1) {
					$item->$small = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_S.jpg';
					$item->imageSmall = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_S.jpg';
				}
				else {
					$item->$small = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_S.jpg';
				}
				
				if ($params->get('imageTimestamp'))
				{
					$item->$small .= $timestamp;
				}
			}

			if (JFile::exists($filepath.'_M.jpg'))
			{
				if($filecount == 1) {
					$item->$medium = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_M.jpg';	
					$item->imageMedium = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_M.jpg';	
				}
				else {
					$item->$medium = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_M.jpg';
				}
				
				if ($params->get('imageTimestamp'))
				{
					$item->$medium .= $timestamp;
				}
			}

			if (JFile::exists($filepath.'_L.jpg'))
			{
				if($filecount == 1) {					
					$item->$large = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_L.jpg';
					$item->imageLarge = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_L.jpg';
				}
				else {
					$item->$large = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_L.jpg';
				}
				
				if ($params->get('imageTimestamp'))
				{
					$item->$large .= $timestamp;
				}
			}

			if (JFile::exists($filepath.'_XL.jpg'))
			{
				if($filecount == 1) {
					$item->$xlarge = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_XL.jpg';
					$item->imageXLarge = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_XL.jpg';
				}
				else {
					$item->$xlarge = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_XL.jpg';
				}
				
				if ($params->get('imageTimestamp'))
				{
					$item->$xlarge .= $timestamp;
				}
			}

			if (JFile::exists($filepath.'_Generic.jpg'))
			{
				if($filecount == 1) {
					$item->$generic = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_Generic.jpg';
					$item->imageGeneric = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_Generic.jpg';
				}
				else {
					$item->$generic = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_Generic.jpg';
				}
				
				if ($params->get('imageTimestamp'))
				{
					$item->$generic .= $timestamp;
				}
			}
		...
		{
			$item->extra_fields = K2ModelItemMultiimages::getItemExtraFields($item->extra_fields, $item);
		}
		...
		{
			$item->attachments=K2ModelItemMultiimages::getItemAttachments($item->id);
		}
		...
		{
			$item->votingPercentage = K2ModelItemMultiimages::getVotesPercentage($item->id);
			$item->numOfvotes = K2ModelItemMultiimages::getVotesNum($item->id);
		...
		//added
		$caption = explode("|", htmlspecialchars($item->image_caption, ENT_QUOTES));
		for($i=0; $i<count($caption); $i++) {
			$capt_n = "image".($i+1)."_caption";
			$item->$capt_n = $caption[$i];
			if($i == 0) {
				$item->image_caption = $caption[$i];
			}
		}		
		
		$credits = explode("|", htmlspecialchars($item->image_credits, ENT_QUOTES));
		for($i=0; $i<count($credits); $i++) {
			$cred_n = "image".($i+1)."_credits";
			$item->$cred_n = $credits[$i];
			if($i == 0) {
				$item->image_credits = $credits[$i];
			}
		}
		//added
    ...
    function prepareJSONItem($item)
    {
		...
		(JFile::exists(JPATH_SITE.DS.'media'.DS.'k2'.DS.'items'.DS.'src'.DS.md5("Image".$item->id).".jpg") && $filecount == 1)
			...
			$images[$images_count] = new JObject;
			unset($images[$images_count]->_errors);

			$images[$images_count]->caption = $caption[$images_count];
			
			$cred_n = "image".$filecount."_credits";
			$images[$images_count]->credits = $credits[$images_count];
			...
			{
				if($filecount > 1) {
					$images[$images_count]->XSmall = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_XS.jpg';
				}
				else {
					$images[$images_count]->XSmall = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_XS.jpg';
				}
			}
			...
			{
				if($filecount > 1) {
					$images[$images_count]->Small = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_S.jpg';
				}
				else {
					$images[$images_count]->Small = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_S.jpg';
				}
			}
			...
			{
				if($filecount > 1) {
					$images[$images_count]->Medium = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_M.jpg';
				}
				else {
					$images[$images_count]->Medium = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_M.jpg';
				}
			}
			...
			{
				if($filecount > 1) {
					$images[$images_count]->Large = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_L.jpg';
				}
				else {
					$images[$images_count]->Large = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_L.jpg';
				}
			}
			...
			{
				if($filecount > 1) {
					$images[$images_count]->XLarge = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_XL.jpg';
				}
				else {
					$images[$images_count]->XLarge = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_XL.jpg';
				}
			}
			...
			{
				if($filecount > 1) {
					$images[$images_count]->Generic = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id)."_".$filecount.'_Generic.jpg';
				}
				else {
					$images[$images_count]->Generic = JURI::base(true).'/media/k2/items/cache/'.md5("Image".$item->id).'_Generic.jpg';
				}
			}
	...
	function getItemExtraFields($itemExtraFields, &$item = null)
	{
		...
		{
			K2ModelItemMultiimages::buildAliasBasedExtraFields($K2ItemExtraFieldsInstances[$itemExtraFields], $item);
	...
	function setDefaultImage(&$item, $view, $params = NULL) {
		if ($view == 'item') {
			$image = 'image1'.$item->params->get('itemImgSize');
		...
		if ($view == 'itemlist') {			
			$image = 'image1'.$params->get($item->itemGroup.'ImgSize');
			$item->image = $item->$image;
	...
	function setDefaultMultiImage(&$item, $view, $params = NULL)
    {
